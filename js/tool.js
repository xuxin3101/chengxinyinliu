...
function getdemopage() {
	...
	if (opage == undefined)
		opage = 1
	http.get("./api/getdemopage.php", function (err, result) {
		var data = result
		if (typeof data == 'string')
			data = JSON.parse(data.trim())
		...
		var index = document.createElement("a");
		index.innerHTML = "首页"
		index.href = "demo.html"
		if (opage != 1) {
			var back = document.createElement("a");
			back.innerHTML = "首页"
			back.href = "demo.html?page=" + parseInt(opage - 1)
			div.appendChild(next)
		}
		...
		if (opage != parseInt(end)) {
			var next = document.createElement("a");
			next.innerHTML = "下一页"
			next.href = "demo.html?page=" + parseInt(opage + 1)
			div.appendChild(next)
		}
